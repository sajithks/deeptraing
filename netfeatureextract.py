...
for kk in params:
    for ii in range(np.shape(net.params[kk][0].data)[0]):
        for jj in range(np.shape(net.params['conv1'][0].data[ii])[0]):
            filt = net.params['conv1'][0].data[ii][jj]     
...
for row in np.int32(np.linspace(0, orimg.shape[0]-1, 315)):
    for col in np.int32(np.linspace(0, orimg.shape[1]-1, 315)):
        feat.append(featmat[:, row, col])
        lab.append(labimg[row, col])
...
#    outimg[ii,:] = rforest.predict(featmat[:,ii,:].T)    
    outimg[ii,:] =rforest.predict_proba(featmat[:,ii,:].T)[:,0]
